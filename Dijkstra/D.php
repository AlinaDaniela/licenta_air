...
class Dijkstra {
        ...
        function Dijkstra(&$ourMap, $infiniteDistance, $ii) {
        ...
        function getResults($to = null) {
                ...
                $ii=2;
                        ...
                        $ourShortestPath[$ii] = array();
                        $endNode = null;
                        $currNode = $ii;
                        $ourShortestPath[$ii][] = $ii;
                        while ($endNode === null || $endNode != $this -> startnode) {
                                $ourShortestPath[$ii][] = $this -> previousNode[$currNode];
                                $endNode = $this -> previousNode[$currNode];
                                $currNode = $this -> previousNode[$currNode];
                        }
                        $ourShortestPath[$ii] = array_reverse($ourShortestPath[$ii]);
                        if ($to === null || $to === $ii) {
                        if($this -> distance[$ii] >= $this -> infiniteDistance) {
                                $foo .= sprintf("no route from %d to %d. \n",$this -> startnode,$ii);
                        } else {
                                $foo .= sprintf('%d => %d = %d [%d]: (%s).'."\n" ,
                                                $this -> startnode,$ii,$this -> distance[$ii],
                                                count($ourShortestPath[$ii]),
                                                implode('-',$ourShortestPath[$ii]));
                        }
                        $foo .= str_repeat('-',20) . "\n";
                                if ($to === $ii) {
                                        break;
...
$dijkstra = new Dijkstra($ourMap, I,$matrixWidth, 2);
